layui.use(['table','layer',"form"],function(){
    ...
    table.on('toolbar(customers)', function(obj){
        ...
        switch(obj.event){
            ...
            case "order":
                ...
                break;
            case "link":
                openLinkManInfoDialog(checkStatus.data);
                break;
            case "recode":
    ...
    function openContactInfoDialog(datas) {
        ...
        var url  =  ctx+"/customer/ContactInfoPage?cusId="+datas[0].id;
        layui.layer.open({
            title : "记录信息展示",
            type : 2,
            area:["900px","700px"],
    ...
    // 打开订单查看页面
    function openLinkManInfoDialog(datas) {
        if(datas.length==0){
            layer.msg("请选择待查看联系人!", {icon: 5});
            return;
        }
        if(datas.length>1){
            layer.msg("暂不支持批量查看!", {icon: 5});
        ...
        var url  =  ctx+"/customer/linkManInfoPage?cusId="+datas[0].id;
        layui.layer.open({
            title : "联系人信息展示",
            type : 2,
            area:["900px","700px"],
